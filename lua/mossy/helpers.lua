...
function h.make_args_range(args, start_arg, end_arg, opts)
  ...
  return function(params)
    ...
    local row, col, end_row, end_col =
      params.range.rstart[1],
      params.range.rstart[2],
      params.range.rend[1],
      params.range.rend[2]
    ...
    local range_start = opts.use_rows and row
      or vim.api.nvim_buf_get_offset(params.buf, row) + col
    local range_end = opts.use_rows and end_row
      or vim.api.nvim_buf_get_offset(params.buf, end_row) + end_col
